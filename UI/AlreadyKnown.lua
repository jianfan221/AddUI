...
f:SetScript("OnEvent", function(self, event, ...)
	if event == "ADDON_LOADED" and (...) == ADDON_NAME then
		if C_AddOns.IsAddOnLoaded("Blizzard_AuctionUI") then
			...
			if C_AddOns.IsAddOnLoaded("Auc-Advanced") and _G.AucAdvanced.Settings.GetSetting("util.compactui.activated") then
		...
		if type(AlreadyKnownSettings) ~= "table" then
			AlreadyKnownSettings = {r = .1, g = .1, b = .7, monochrome = false}		--颜色
		end
