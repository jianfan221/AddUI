...
ns.event("PLAYER_LOGIN", function()
	if AddUIDB.cdset then
		...
		ns.hook(EssentialCooldownViewer, "RefreshData", function(self,event)
			for itemFrame in self.itemFramePool:EnumerateActive() do
				CreateShadow(itemFrame)
			end
		end)
		ns.hook(UtilityCooldownViewer,"RefreshData", function(self,event)
			for itemFrame in self.itemFramePool:EnumerateActive() do
				CreateShadow(itemFrame)
			end
		end)
		
		ns.hook(BuffBarCooldownViewer,"RefreshData", function(self,event)
			for itemFrame in self.itemFramePool:EnumerateActive() do
				CreateShadow(itemFrame)
			end
		end)
	...
	if AddUIDB.cdcenter then
		...
		ns.hook(BuffIconCooldownViewer, "OnUnitAura", function(self)
			ns.SetBuffIconPoint(self)
		end)
		ns.hook(BuffIconCooldownViewer, "RefreshLayout", function(self)
			ns.SetBuffIconPoint(self)
		end)
		ns.hook(BuffIconCooldownViewer, "RefreshData", function(self,event)
			ns.SetBuffIconPoint(self)
		...
		ns.hook(EditModeManagerFrame, "OnSystemPositionChange", function(self,frame)
